...
export default class WordWizardScene extends Phaser.Scene {
  ...
  private createTypingMode(currentWord: { word: string; description: string }) {
    ...
    const tilesY  = isMobile ? this.scale.height * 0.50 : this.scale.height / 2 - 20;
    const inputY  = isMobile ? this.scale.height * 0.62 : this.scale.height / 2 + 110;
    const buttonY = isMobile ? this.scale.height * 0.70 : this.scale.height / 2 + 200;
