...
const CATEGORY_BG_COLORS: Record<string, { top: number; bottom: number }> = {
  BASIC:    { top: 0x1a4a2e, bottom: 0x0d2b1a },
  NORMAL:   { top: 0x1a3a5c, bottom: 0x0d1f3a },
  HARD:     { top: 0x5c1a1a, bottom: 0x3a0d0d },
  ADVANCED: { top: 0x3a1a00, bottom: 0x261200 },
  EXPERT:   { top: 0x1a0a2e, bottom: 0x0d0519 },
};
...
export default class WordWizardScene extends Phaser.Scene {
  ...
  private inventory: Record<string, number> = {
    time_freeze: 0, second_chance: 0, score_booster: 0,
  };
  ...
  // 📱 Keep a reference so we can cleanly remove the listener on shutdown
  private _viewportResizeHandler?: () => void;
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // 🎨 Per-category gradient background
  // ─────────────────────────────────────────────────────────────────────────
  private createCategoryBackground() {
    ...
    // Gradient via stacked horizontal rects
    const steps = 80;
    ...
    for (let i = 0; i < steps; i++) {
      ...
      g.fillRect(0, Math.floor((i / steps) * height), width, Math.ceil(height / steps) + 1);
    }
    ...
    // Top / bottom color strips
    g.fillStyle(accent, 0.18); g.fillRect(0, 0,          width, 8);
    g.fillStyle(accent, 0.10); g.fillRect(0, 8,          width, 4);
    g.fillStyle(accent, 0.18); g.fillRect(0, height - 8, width, 8);
    g.fillStyle(accent, 0.10); g.fillRect(0, height - 12,width, 4);
  }

  // ─────────────────────────────────────────────────────────────────────────
  // 📱 Prevent the mobile keyboard from resizing / reflowing the Phaser canvas
  //
  // Strategy:
  //   • Capture the visual viewport height BEFORE the keyboard opens.
  //   • Fix the canvas to that height via CSS so it never shrinks.
  //   • Use visualViewport "resize" to detect keyboard open/close.
  //   • Restore everything when the keyboard is dismissed.
  // ─────────────────────────────────────────────────────────────────────────
  private lockCanvasForMobileKeyboard() {
    const canvas      = this.game.canvas;
    const lockedH     = window.visualViewport?.height ?? window.innerHeight;

    // Lock canvas size
    canvas.style.height    = `${lockedH}px`;
    canvas.style.maxHeight = `${lockedH}px`;
    canvas.style.overflow  = "hidden";

    // Prevent body scroll / reflow
    ...
    const onViewportResize = () => {
      ...
      if (vvh < lockedH - 50) {
        // Keyboard opened — keep canvas pinned at top
  ...
  async create() {
    ...
    // 📱 Lock canvas layout against the mobile keyboard
    if (this.scale.width < 768) {
      this.lockCanvasForMobileKeyboard();
    }

    try {
      this.categoryProgress = await getAllCategoryProgress(
        this.userId, "WordWizard", WORDWIZARD_CATEGORIES
      );
    } catch {
    ...
    if (!this.isTypingMode) {
      this.input.on("dragstart", (_: any, go: any) => {
        ...
        for (const slot of this.slots) {
          if ((slot as any).filled === go) { (slot as any).filled = null; break; }
        }
      });
  ...
  private createLevelInfoText() {
    ...
    this.add.text(
      levelInfoX, levelInfoY,
      `${this.getCategoryDisplayName()}\nLevel ${this.currentLevelInCategory}`,
      {
        fontSize: levelInfoFontSize, fontStyle: "bold",
        color: "#ffffff", backgroundColor: "#4a5568dd",
  ...
  private async handleSceneEnd() {
    ...
    if (this.typingInputField) { this.typingInputField.remove(); this.typingInputField = undefined; }
    this.unlockCanvasForMobileKeyboard();
  }
  ...
  private clearLevelElements() {
    ...
    if (this.typingInputField)   { this.typingInputField.remove();   this.typingInputField   = undefined; }
    if (this.typingSubmitButton) { this.typingSubmitButton.destroy(); this.typingSubmitButton = undefined; }
    if (this.staticLettersText)  { this.staticLettersText.destroy();  this.staticLettersText  = undefined; }
  }
  ...
  //   Smaller tiles & slots on mobile so everything fits without overlap
  // ─────────────────────────────────────────────────────────────────────────
  private createDragAndDropMode(currentWord: { word: string; description: string }) {
    ...
    // ── Slots ──────────────────────────────────────────────────────────────
    // Spacing reduced: 52 px (was 70) on mobile → tiles fit even for long words
    const slotSpacing = isMobile ? 52 : 130;
    // Push slots into the middle of the UPPER half so tiles below have room
    const slotY   = isMobile ? this.scale.height * 0.46 : this.scale.height * 0.28;
    const startX  = centerX - ((currentWord.word.length - 1) * slotSpacing) / 2;

    for (let i = 0; i < currentWord.word.length; i++) {
      const slot = createSlot(this, startX + i * slotSpacing, slotY);
      // Scale the slot graphic itself down on mobile
      if (isMobile) (slot as any).setScale?.(0.52);
    ...
    const tileGap        = isMobile ? 5 : 20;

    // Max tile: 40 px on mobile (was 58) keeps everything compact
    const maxTileSize = isMobile ? 40 : 110;
    const minTileSize = isMobile ? 20 : 60;
    const rawTileSize = Math.floor(
      (availableWidth - tileGap * (totalTileCount + 1)) / totalTileCount
    );
    const tileSize    = Math.min(maxTileSize, Math.max(minTileSize, rawTileSize));
    const tileSpacing = tileSize + tileGap;
    const tileScale   = tileSize / 64; // base tile art is 64 px
    ...
    // Tiles in the lower-middle area — well clear of info text AND the keyboard
    const tileY      = isMobile ? this.scale.height * 0.66 : this.scale.height * 0.51;
    ...
    letters.forEach((letter, i) => {
      ...
      if (isMobile) tile.setScale(tileScale);
  ...
  private createMultipleChoiceMode(currentWord: { word: string; description: string; choices?: string[] }) {
    const isMobile = this.scale.width < 768;
    const centerX  = this.scale.width / 2;
    // Keep buttons in the upper 55% so they are never hidden by the keyboard
    const centerY  = isMobile ? this.scale.height * 0.50 : this.scale.height / 2;

    const choices  = currentWord.choices ?? [currentWord.word, "Option B", "Option C"];
    const shuffled = Phaser.Utils.Array.Shuffle([...choices]);

    const btnWidth   = isMobile ? this.scale.width * 0.78 : 600;
    const btnHeight  = isMobile ? 50 : 72;        // was 64 — slightly shorter on mobile
    const btnRadius  = 14;
    const btnSpacing = isMobile ? 60 : 86;         // was 76 — tighter on mobile
    const totalH     = btnSpacing * shuffled.length;
    const startY     = centerY - totalH / 2 + 28;
    const fontSize   = isMobile ? "14px" : "22px"; // was 17px
    ...
    shuffled.forEach((choice, i) => {
      ...
      const label = this.add
        .text(0, 0, choice, {
          ...
          wordWrap: { width: btnWidth - 32 }, align: "center",
        })
        .setOrigin(0.5);
  ...
  private showMCFeedback(correct: boolean, x: number, y: number) {
    ...
    this.tweens.add({
      targets: feedbackText, alpha: 1, y: y - 80, duration: 250, ease: "Power2",
      onComplete: () => {
        this.tweens.add({
          targets: feedbackText, alpha: 0, duration: 400, delay: 300,
  ...
  //   All content is placed in the top ~55% of the screen so the keyboard
  //   (which takes the bottom ~45%) never covers anything.
  // ─────────────────────────────────────────────────────────────────────────
  private createTypingMode(currentWord: { word: string; description: string }) {
    const isMobile = this.scale.width < 768;
    const centerX  = this.scale.width / 2;

    // Safe vertical band: top 20% → 56% of canvas height
    // The keyboard on an average phone occupies roughly the bottom 44%.
    const safeTop    = isMobile ? this.scale.height * 0.20 : this.scale.height * 0.28;
    const safeMid    = isMobile ? this.scale.height * 0.38 : this.scale.height * 0.46;
    const safeBtn    = isMobile ? this.scale.height * 0.49 : this.scale.height * 0.57;

    // Shuffled letter tiles
    const letters = Phaser.Utils.Array.Shuffle(currentWord.word.split(""));
    const letterDisplay = createStaticLetterDisplay(this, letters, centerX, safeTop);
  ...
  private async handleTypingSubmit() {
    ...
    if (answer === target) {
      ...
      try {
        localStorage.setItem("wordwizard_session_score", this.score.toString());
        localStorage.setItem("wordwizard_streak", this.consecutiveLevelsCompleted.toString());
      } catch (e) { console.error("Failed to save score/streak:", e); }
      ...
      this.time.delayedCall(800, async () => { await this.handleFail(); });
    }
  }
  ...
  private showTypingFeedback(correct: boolean) {
    ...
    // Show feedback ABOVE the letter tiles so it never overlaps the input field
    const isMobile  = this.scale.width < 768;
    const feedbackY = isMobile ? this.scale.height * 0.11 : this.scale.height / 2 - 150;

    const feedbackText = this.add
      .text(this.scale.width / 2, feedbackY, correct ? "✓ CORRECT!" : "✗ WRONG!", {
    ...
    this.tweens.add({
      ...
      onComplete: () => {
        this.tweens.add({
          targets: feedbackText, alpha: 0, duration: 500, delay: 200,
          onComplete: () => feedbackText.destroy(),
        });
      },
    });
  ...
  private trySnap(tileContainer: Phaser.GameObjects.Container) {
    // Slightly tighter snap radius to match the smaller tile size on mobile
    const isMobile  = this.scale.width < 768;
    const threshold = isMobile ? 45 : 60;
    let snapped = false;

    for (const slot of this.slots) {
      if (!(slot as any).filled) {
        const dist = Phaser.Math.Distance.Between(
          tileContainer.x, tileContainer.y, slot.x, slot.y
        );
        if (dist < threshold) {
    ...
    if (!snapped) {
      ...
      this.tweens.add({ targets: tileContainer, x: home.x, y: home.y, duration: 200, ease: "Back.easeOut" });
    }
  ...
  private showDecoyRejection(tile: Phaser.GameObjects.Container) {
    ...
    this.tweens.add({
      targets: tile, x: tile.x + 12, duration: 60, yoyo: true, repeat: 3,
      ease: "Sine.easeInOut",
      onComplete: () => {
        this.tweens.add({ targets: tile, x: home.x, y: home.y, duration: 200, ease: "Back.easeOut" });
      },
    });

    const flash = this.add
      .text(tile.x, tile.y - 40, "✗", {
        fontSize: "28px", fontStyle: "bold", color: "#ef4444",
        stroke: "#000000", strokeThickness: 4,
      })
      .setOrigin(0.5).setDepth(200);

    this.tweens.add({
      targets: flash, alpha: 0, y: flash.y - 20, duration: 500,
  ...
  private async checkComplete() {
    ...
    const answer = this.slots.map((s) => ((s as any).filled as any)?.letter || "").join("");
    const target = this.level.words[0].word;
  ...
  private async handleFail() {
    ...
    if (this.isSecondChanceActive) {
      ...
      if (this.isTypingMode) {
        if (this.typingInputField) { this.typingInputField.value = ""; this.typingInputField.focus(); }
      } else { this.resetTiles(); }
      this.resetTimer();
    ...
    if (hasSecondChance) {
      ...
      } catch (e) { console.error("Failed to reset streak/score:", e); }
  ...
  private showSecondChanceMessage() {
    const msg = this.add
      .text(this.scale.width / 2, this.scale.height / 2, "♻️ Second Chance!", {
        font: "bold 48px Arial", color: "#00ff99", stroke: "#006644", strokeThickness: 6,
      })
      .setOrigin(0.5).setDepth(200);
    this.tweens.add({
      targets: msg, alpha: 0, duration: 1500, ease: "Cubic.easeOut",
  ...
  private async showGameOverScreen() {
    ...
    const gameOverText = this.add
      .text(centerX, centerY - 40, "💀 Game Over!", {
        font: "bold 56px Arial", color: "#f87171", stroke: "#7f1d1d", strokeThickness: 6,
      })
      .setOrigin(0.5).setDepth(10);

    const retryText = this.add
      .text(centerX, centerY + 20, "Returning to map...", {
        font: "28px Arial", color: "#fff",
      })
      .setOrigin(0.5).setDepth(10);

    this.tweens.add({
      targets: [gameOverText, retryText],
      alpha: 0, duration: 1500, ease: "Cubic.easeOut",
      onComplete: () => {
        gameOverText.destroy();
        retryText.destroy();
        this.cameras.main.fadeOut(600, 0, 0, 0);
        this.time.delayedCall(650, () => { window.location.href = "/wordwizardmap"; });
      },
    });
  }
  ...
  private resetTiles() {
    ...
    this.slots.forEach((slot) => { (slot as any).filled = null; });
  }
  ...
  private async showWinScreen() {
    ...
    let s = 1;
    if (isMobile) {
      const scaleW = this.scale.width  / 1600;
      const scaleH = this.scale.height / 900;
      s = Math.min(scaleW, scaleH) * 1.1;
    ...
    const correctText = this.add
      .text(centerX, centerY + correctYOffset, "✨ AMAZING! ✨", {
        ...
        color: "#fbbf24", stroke: "#d97706",
        strokeThickness: isMobile ? 6 : 8,
      })
      .setOrigin(0.5).setDepth(10).setScale(0);

    this.tweens.add({ targets: correctText, scale: isMobile ? 1.0 : 1.2, duration: 600, ease: "Bounce.easeOut" });
    this.tweens.add({
      targets: correctText,
    ...
    const scoreText = this.add
      .text(centerX, centerY + scoreYOffset, `🏆 Total Score: ${this.score} 🏆`, {
        ...
        color: "#ffffff", stroke: "#8b5cf6",
        strokeThickness: isMobile ? 4 : 6,
      })
      .setOrigin(0.5).setDepth(10).setAlpha(0);

    this.tweens.add({ targets: scoreText, alpha: 1, duration: 500, delay: 300, ease: "Power2" });
    this.tweens.add({
      targets: scoreText,
        ...
        onClick: async () => {
          ...
          if (this.currentLevelInCategory < 15) {
            ...
            try {
              localStorage.setItem("wordwizard_session_score", "0");
              localStorage.setItem("wordwizard_streak", "0");
            } catch (e) { console.error("Failed to reset score/streak:", e); }
  ...
  private async unlockNextLevel() {
    ...
    if (nextLevelInCategory > currentUnlocked) {
      try {
        await saveCategoryLevel(this.userId, "WordWizard", this.currentCategoryId, nextLevelInCategory);
        this.categoryProgress[this.currentCategoryId] = nextLevelInCategory;
        window.dispatchEvent(new CustomEvent("levels:updated"));
      } catch (e) { console.error("Failed to save category progress:", e); }
    }
  }
  ...
  private createTimerAndHUD() {
    ...
    const createInventoryRow = (
      y: number, emoji: string, label: string,
      ...
      const btnWidth = 60, btnHeight = 28;
      ...
      btnBg.on("pointerdown", async () => { await this.handleUsePowerUp(type); });
  ...
  private async fetchInventoryAndUpdateHUD() {
    ...
    } catch (err) { console.error("Failed to fetch power-up inventory:", err); }
  }
  ...
  private async handleUsePowerUp(type: "time_freeze" | "second_chance" | "score_booster") {
    ...
    if (qty <= 0) {
      const warning = this.add
        .text(this.scale.width / 2, this.scale.height / 2, "⚠️ No power-up available", {
          ...
          color: "#ffffff", stroke: "#ef4444", strokeThickness: 4,
        })
        .setOrigin(0.5).setDepth(200);
    ...
    if (type === "second_chance" && this.isSecondChanceActive) {
      const warning = this.add
        .text(this.scale.width / 2, this.scale.height / 2, "♻️ Second Chance already active!", {
          fontFamily: '"Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif',
          fontSize: "22px", fontStyle: "bold",
          color: "#ffffff", stroke: "#10b981", strokeThickness: 4,
        })
        .setOrigin(0.5).setDepth(200);
      this.time.delayedCall(1600, () => warning.destroy());
      return;
    ...
    try {
      ...
      await this.fetchInventoryAndUpdateHUD();
    } catch (err) { console.error("Error using power-up:", err); }
  }
  ...
  private resetTimer() {
    ...
    if (this.timerText) {
      ...
      this.timerText.setColor(
        this.timeRemaining <= 10 ? "#ef4444" :
        this.timeRemaining <= 15 ? "#f59e0b" : "#ffffff"
      );
    }
      ...
      callback: async () => {
        ...
        if (this.timerText) {
          ...
          if (this.timeRemaining <= 5) {
            ...
            this.tweens.add({
              targets: this.timerText, scale: { from: 1, to: 1.2 }, duration: 300, yoyo: true,
            });
